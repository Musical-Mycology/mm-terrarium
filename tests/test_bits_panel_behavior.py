"""Behavioral test for the Bits panel in console/static/console.js.
...
843 passing tests. This drives the real shipped console.js against a DOM stub
under Node, and its load-bearing scenario is that a state_changed event does
NOT rebuild the Bits card list, the same defect class as the Room strip's and
the Trigger panel's.
...
TEST_SCRIPT = ROOT / "tests" / "js" / "bits_panel_behavior.test.js"
...
def test_bits_panel_behavior():
    ...
    assert result.returncode == 0, (
        "bits_panel_behavior.test.js failed:\n"
        f"stdout:\n{result.stdout}\n"
        f"stderr:\n{result.stderr}"
    )
