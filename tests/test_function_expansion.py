...
data: the exact cue sequence a function produces is assertable with no Arco, no
...
def _function(script, target=FunctionTarget.ROOM):
    return Function(
...
def test_an_empty_script_expands_to_nothing():
    assert expand_script(_function(()), AT, ["sim-room"]) == []
...
def test_each_step_carries_its_offset_added_to_at():
    ...
    out = expand_script(fn, AT, ["sim-room"])
...
def test_target_is_substituted_with_the_resolved_dev():
    fn = _function((ScriptStep(0.0, (TARGET, 0xB0, 74, 127)),))
    out = expand_script(fn, AT, ["sim-room"])
...
def test_target_fans_out_to_every_resolved_dev():
    ...
    out = expand_script(fn, AT, ["sim-room", "ie1", "ie2"])
...
def test_a_room_addressed_step_is_left_for_resolve_dev_downstream():
    ...
    fn = _function((ScriptStep(0.0, (ROOM, 0xB0, 74, 127)),),
                   target=FunctionTarget.DEVICE)
    out = expand_script(fn, AT, ["ie1"])
...
def test_a_play_cue_step_keeps_its_name_and_params_and_gains_the_dev():
    fn = _function((ScriptStep(0.0, PlayCue(TARGET, "click", "soft")),),
                   target=FunctionTarget.DEVICE)
    out = expand_script(fn, AT, ["ie1"])
