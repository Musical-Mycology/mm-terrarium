...
builds one `LightSession` per joined device and one per Room fixture, drives
...
```ascii
     ...
     └───────────────┘ └─────────────────┘      │                
             ...
             │                  ▼               ▼                
             │           ┌──────────────┐ ┌────────────┐         
        /<dev>/leds      │Console strip │ │ArtNet sink │         
             │           │              │ │            │         
             │           └──────────────┘ └────────────┘         
             │                                  │                
             ▼                                  │                
...
```ascii
Player flow, hello to complete (lobby handshake, 2026-09-11)
...
[ SETUP holds registration open; --setup-seconds widens the window ]
       |                                                 |                                               |
       |                                                 |<------------/ie1/leds invite flash------------|
       |                                                 |                                               |
       |<-------------------/ie1/leds--------------------|                                               |
       |                                                 |                                               |
[ a double tap, or two taps within 1.5s, joins with no node needed ]
       |                                                 |                                               |
       |--------------/game/tap count >= 2-------------->|                                               |
       |                                                 |                                               |
       |                                                 |------------------/game/tap------------------->|
...
[ a Room fixture bell climbs the A major scale on Control's own Arco voice ]
       ...
       |                                                 |-----------------/game/start------------------>|
       |                                                 |                                               |
[ request_start accepts; SETUP -> RUNNING; Room fixtures flash green x1 ]
       |                                                 |                                               |
       |-------------------/game/tilt------------------->|                                               |
       |                                                 |                                               |
       |                                                 |------------------/game/tilt------------------>|
...
└────────────────────────────────────────────┘
         ...
         └───────────────────┘                
                  ...
                  ▼                           
     ┌───────────────────────────┐            
     │4. room stack: Room bridge │            
     │                           │            
     └───────────────────────────┘            
                  │                           
                  ▼                           
    ┌───────────────────────────────┐         
    │5. room stack: Room simulators │         
    │                               │         
    └───────────────────────────────┘         
                  │                           
                  ▼                           
...
- **Closed 2026-09-25 (the VENUE Room entry below):** an `[[artnet]]`-covered
  fixture now spawns no simulator and is never waited for, and an
  all-Art-Net Room loads with no bound device. The shipped entry point
  always ALSO spawns a WebSim simulator for an `[[artnet]]` fixture.
