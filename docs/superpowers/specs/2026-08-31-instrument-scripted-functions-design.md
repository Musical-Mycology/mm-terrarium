...
| **Mid-execution redirect (2026-08-31, Chris)** | **Bits are NOT migrated in this slice.** TestBit/MetronomeBit keep their scripted functions, scripts and all, living in mm-terrarium; the content move to instruments is deferred ("we will redirect that at a later time"). Consequences: Bit-side SCRIPTED validation stays exactly as it was (scripts allowed; an EMPTY script is additionally legal and means name-fire delegation); the reserved-name refusal applies ONLY to instrument declarations (TestBit's existing `stop` must keep loading); the firing ladder gains a first rung -- **Bit-declared script (non-empty) → built-in → target instrument's function → logged no-op**. The MetronomeBit row below and the TestBit migration text in section 3 are deferred accordingly; the instrument-authored layer (sections 1-2, TOML + TUNESHROOM vocabulary) still lands as the forward path |
| MetronomeBit's scripts (planning-time gap) | Its content moves to the instruments it plays: device-facing scripts (fireworks_player, fail_player, metro_pulse_player, metro_recovery) onto the code-defined `TUNESHROOM` instrument ("it requires a tuneshroom to play" -- Chris); Room-facing scripts (fireworks_room, fail_room, finale, metro_downbeat, metro_click, metro_pulse_room) into `terrarium.toml` on the room instruments. Instrument functions are therefore authored **where the instrument is defined**: TOML for config-declared instruments, Python for code-defined ones. Existing function names are kept (no `_player`/`_room` collapse this slice); room-TOML override of a code-defined instrument's functions is deferred |
